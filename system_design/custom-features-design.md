...
-   **第一阶段 (召回)**:
    ...
    -   **实现**: 并行执行两种召回策略，并将结果合并去重：
        1.  **关键词召回 (BM25)**: 利用 OpenSearch 自带的BM25算法，进行传统的关键词匹配。对包含大量专业术语的运维场景非常有效。
        2.  **向量召回**: 使用部署在 PAI-EAS 上的 `bge-m3` 模型，将用户问题向量化后在 OpenSearch 中进行向量相似度检索。
-   **第二阶段 (精排)**:
    -   **目标**: 对第一阶段召回的候选文档列表（例如 Top 50）进行更精细、更昂贵的计算，得到最准确的相关性排序。
    -   **实现**: 调用部署在 PAI-EAS 上的 `bge-reranker-v2-m3` 模型。它会接收用户问题和每一篇候选文档，输出一个精确的相关性分数。
...
**关联工作流**：
*   该IDP流程是**知识处理流水线**（由OSS触发的函数计算FC）的核心组成部分，是所有文档入库前的必经步骤，取代了传统的`pdfplumber`或`BeautifulSoup`等单一解析库。
